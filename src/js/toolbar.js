...
import { initHeaderEditor } from './headerEditor.js'; 
...
function postAnnouncement() {
    ...
    if(!text) return;
    const newItem = { type: 'alert', page: state.currentPage || 'home', position: -10, content: `<h3><i class="fas fa-exclamation-triangle"></i> Important</h3><p>${text}</p>`, styles: { padding: "20px", maxWidth: "800px", margin: "20px auto" } };
    state.items.unshift(newItem); triggerOptimisticUpdate();
    document.getElementById('emergency-text').value = ''; document.getElementById('emergency-modal').classList.add('hidden');
}

function openSectionsManager() { renderSectionsTable(); document.getElementById('sections-modal').classList.remove('hidden'); }
...
function renderSectionsTable() {
    ...
    sortedItems.forEach(item => {
        // HIDE META ITEMS
        if (item.type === 'header_config') return; 
...
function setupRichTextEditor() {
    ...
    document.getElementById('editor-link-btn').addEventListener('click', async (e) => { e.preventDefault(); const url = await ask("Enter URL:", "https://"); if (url) document.execCommand('createLink', false, url); });
}
